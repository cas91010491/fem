...
class Contact:
    def __init__(self,slave, master, kn=1.0, kt=None,
        ...
        self.force_iter = np.zeros((nsn,3))
        ...
        univ_t1t_name = "testing_universal_sp_model-shape-512-512-bs-64"
        self.univ_t1t2_model = SurfacePointsModelForOnePatch(name=univ_t1t_name)
    ...
    def getCandidates_univ(self, u,CheckActive = False,TimeDisp = False,tracing = False):
        ...
        if len(proximal_candidates) > 0:
            predictions_for_t1t2 = self.univ_t1t2_model.predict( xs_forANN[proximal_candidates] , verbose=0 )
            set_trace()

            model_predictions = self.patch_classifier.Predict(points=xs_forANN[proximal_candidates], n=9)
            self.candids[proximal_candidates] = [mod_pred[mod_pred[:,1]>1e-6,0]  for i,mod_pred in enumerate(proximal_candidates)]
    ...
    def getfintC(self, Model, DispTime = False, useANN = False,tracing = False):
        ...
        if useANN:
            ...
            for i_patch, patch in enumerate(all_patches_obj):
                ...
                if num_nodes_in_patch>0:
                    # predictions_for_t1t2 = patch.MinDistANN( xs_forANN[nodes_in_patch] , verbose=0 )
                    predictions_for_t1t2 = self.univ_t1t2_model( xs_forANN[nodes_in_patch] , verbose=0 )
                    # set_trace()
                    # T1[nodes_in_patch,i_patch] = predictions_for_t1t2[:,0]
                    # T2[nodes_in_patch,i_patch] = predictions_for_t1t2[:,1]
        ...
        for idx in range(self.nsn):
            if self.actives[idx] is not None:
                ...
                if is_patch_correct:        # if patch changed
                    node_id = self.slaveNodes[idx]
                    self.proj[idx] = (patch_id, t[0], t[1], gn)
                    self.force_iter[idx] = fintC[:3]
                    if changed:
                        eventList_iter.append(str(node_id)+": "+str(self.actives[idx])+"-->"+str(patch_id))
                    if gn>0:
                        if changed:
    ...
    def printStates(self,only_actives=True,veredict=False):
        ...
        active_slaves  = []
        active_patches = []
        candid_patches = [] # to (maybe) plot in a different color
            ...
            if len(self.candids[idx])>0:
                ...
                if veredict:
                    ...
                    if not only_actives or active is not None:
                        print("node "+str(idx)+"("+str(slavenode)+"): \tcandidates :",candids,
                            "" if active is None else ("\tactive: "+str(active)+": "+str(self.t1t2cache[idx])) )
                
                
                        active_slaves.append(idx)
                        if active not in active_patches:
                            active_patches.append(active)
            
        # self.plot(active_patches,active_slaves)


    def plot(self,patches,nodes):
        import matplotlib.pyplot as plt
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        f = 10*self.force_iter

        if patches=="all": patches = range(len(self.masterSurf.patches))
        xs = self.xs[nodes]

        ax.scatter(xs[:,0],xs[:,1],xs[:,2],c="red")

        for i_n,node in enumerate(nodes):
            ax.quiver(xs[i_n,0],xs[i_n,1],xs[i_n,2],f[node,0],f[node,1],f[node,2])


        for patch in patches:
            self.masterSurf.patches[patch].plot(axis=ax,label=True,color=(0.0,0.0,1.0,0.6))

        for patch in range(96):
            if patch not in patches:
                self.masterSurf.patches[patch].plot(axis=ax,ref=8,color=(0.5,0.5,0.5,0.3))
