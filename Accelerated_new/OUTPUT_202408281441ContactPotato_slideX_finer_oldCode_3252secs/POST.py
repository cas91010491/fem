...
def plot4D(model,TIMES,UU,SED, ax=None, undef=False):
    ...
    u = UU[0]
    # sedi = sed[0]
    model.bodies[0].plot(ax,u= u,sed = None,ref=4)
    ...
    def update(UU,sed):
        ...
        slaveNodes = model.bodies[0].SelectFlatSide("-z")

        model.bodies[0].plot(ax,u= u,plotWhat="all",ref=4,nodes=slaveNodes)
    ...
    time_slider.on_changed(lambda val: update(UU, 0))
