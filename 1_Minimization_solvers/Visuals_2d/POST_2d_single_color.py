...
def plot4D(model,TIMES,UU,SED, ax=None, undef=False):
    ...
    if ax is None:
        ...
        ax.set_aspect("equal")
        ax.text2D(-0.4, 0.9, "time: "+str(round(0.0,12)), transform=ax.transAxes,backgroundcolor=(1.0,1.0,1.0,0.7),zorder=1000000,fontsize='x-large')
        ax.axis('off')
    ...
    def update(UU,sed):
        ...
        # print("idx =",idx)
        time = TIMES[idx]
        ax.text2D(-0.4, 0.9, "time: "+str(round(time,12)), transform=ax.transAxes,backgroundcolor=(1.0,1.0,1.0,0.7),zorder=1000000,fontsize='x-large')
        u = UU[idx]
        sedi = sed[idx]
        model.bodies[0].plot(ax,u= u,sed = None,ref=1)
