...
class Contact:
    ...
    def getCandidatesANN(self, u,CheckActive = False,TimeDisp = False):
        ...
        proximal_candidates = np.where(predicted_signed_distance<0.1)[0]   # Slave nodes close to the master surface
    ...
    def getfintC(self, Model, DispTime = False, useANN=False):
        ...
        if useANN:
            ...
            for i_patch, patch in enumerate(all_patches_obj):
                ...
                if len(nodes_in_patch)>0:
                    ...
                    T1_mask = (T1[nodes_in_patch,i_patch]<1.1).multiply(T1[nodes_in_patch,i_patch]>-0.1)
                    T2_mask = (T2[nodes_in_patch,i_patch]<1.1).multiply(T2[nodes_in_patch,i_patch]>-0.1)
                    self.candids_sparse[nodes_in_patch,i_patch] =  T1_mask.multiply(T2_mask).todense()
        ...
        for idx in range(self.nsn):
            ...
            if self.actives[idx] is not None:
                ...
                if useANN:
                    self.candids[idx] = self.candids_sparse[idx,:].nonzero()[1]
                ...
                while not is_patch_correct:
                    patch = surf.patches[patch_id]
                    t0 = [T1[idx,patch_id],T2[idx,patch_id]] if useANN else None
                ...
                if is_patch_correct:        # if patch changed
                    ...
                    self.actives[idx] = patch_id
                    self.actives_sparse[idx,:] = 0
                    self.actives_sparse[idx,patch_id] = 1
    ...
    def getKC(self, Model, DispTime = False, useANN=False):
        if DispTime: ti = time.time()
        surf = self.masterSurf
        sBody = self.slaveBody
