...
class FEModel:
    ...
    def setReferences(self,OUT = False):
        u_ref = self.u.copy()
        ...
        if OUT:
            return [u_ref,u_temp_ref, fint_ref, alpha_ref, new_ref, xs_ref,act_ref,cand_ref]
        else:
            self.REF = [u_ref,u_temp_ref, fint_ref, alpha_ref, new_ref, xs_ref,act_ref,cand_ref]

    def getReferences(self, alphas=True,actives=False, IN = False):
        ...
        self.u = REF[0].copy()
        self.u_temp = REF[1].copy()
        self.fint = REF[2].copy()
        alpha_ref, new_ref, xs_ref,act_ref,cand_ref = REF[3:]
        # self.u_temp = np.array(self.u)
    ...
    def get_K(self, DispTime = False):
        ...
        for body in self.bodies:
            # Keep in mind that there are different functions for this. Test on large models! big cpp function might be the best
    ...
    def solve_NR(self,tol=1e-10,maxiter=10,plotIters=False):
        ...
        while RES>tol:
            ...
            # if abs(0.43-self.t)<1e-4:
            # if self.t>0.42+1e-3:
            #     self.contacts[0].printStates()
                # self.plotNow()
                # set_trace
    ...
    def Solve(self, t0 = 0, tf = 1, TimeSteps = 1, max_iter=10 , recover = False, ForcedShift = False):
        ...
        if recover:
            ...
            self.REF,t, dt, ti = pickle.load(open("OUTPUT_202409121631ContactPotato_down_Indent+SlideY/"+"RecoveryData.dat","rb"))
        ...
        while t+dt < tf+1e-4:
            ...
            if DoMinimization:
                ...
                self.u_temp = self.u.copy()  # copy of 'u' so that solution is directly used in NR
                ref_min = self.setReferences(OUT=True)

            converged = self.solve_NR(tol=tolerance,maxiter=max_iter)

            if DoMinimization and not converged:
                # If Newton doesn't manage to finish it off, I just continue with that I have from the minimization (ref_min)
                self.getReferences(IN=ref_min)
                converged = True


            if not converged or redo_count>4:
                print('Increment failed to converge !!!!! Redoing half')
                t -= dt
                ...
                for ic, ctct in enumerate(self.contacts):
                    ...
                    # ctct.getCandidates_univ(self.u, CheckActive = True, TimeDisp=False,tracing=tracing)    # updates Patches->BSs (always) -> candidatePairs (on choice)
