...
def dms(u,X,C,EA,dofs):
    ...
    for (i,j) in C:
        ...
        dms[np.ix_(dofij)]+=EA*((L-L0)/L0**2)*dLdu
    return dms

def findiff(u0,X,C,EA, du = 1e-8):
    u0_flat = u0.ravel()
    m0 = ms(u0,X,C,EA)
...
def plot(u,X,C):
    ...
    for (i,j) in C:
        ...
        ax.plot([Xa[0],Xb[0]],[Xa[1],Xb[1]], c="black")
        ax.plot([xa[0],xb[0]],[xa[1],xb[1]], c="blue")

    plt.show()

u = np.array([[0.1,0.2],[0.2,0.1],[0.3,0.4]])
f1 = dms(u,X,C,1,dofs)
...
# ax.plot([x0,xf],[f1,f1])
for du in np.logspace(-10,-6,num=5, endpoint=False):
    f2 = findiff(u,X,C,1,du=du)
    diff = norm(f2-f1)
    ax.scatter(du,diff)
ax.set_xscale("log")
plt.show()
