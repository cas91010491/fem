...
tol_RM=1e-12;
%subdivide return mapping in n_ll increments
n_ll=10;
...
for incr=1:incr_tot
    ...
    [m_new,f_2,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,u,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv,n_ll);
    ...
    while (m_old-m_new)>tol*abs(m_old)
        ...
        [m_3,f_3,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,ux,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv,n_ll);
        ...
        if signal1==0
            ...
            [m_2,f_2,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,ux,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv,n_ll);
...
figure(1)
hold on
plot([0;results(:,1)],[0;results(:,2)],'b:','linewidth',2)

figure(2)
hold on
plot([0;results(:,1)],[0;results(:,3)],'k:','linewidth',2)

figure(3)
hold on
plot([0;results(:,1)],[0;results(:,4)],'m:','linewidth',2)


function [m,f,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,u,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv,n_ll)
...
for ii=1:length(con_mat(:,1))
    ...
    u_all_conv=u_conv(a_mat(1:3:24-2));
    v_all_conv=u_conv(a_mat(2:3:24-1));
    w_all_conv=u_conv(a_mat(3:3:24));
    ...
    for jj=1:8
        ...
        if yieldfunction<0
            ...
            for ll=1:n_ll
                ...
                F=[dNdXYZ(1:3:24-2)'*(u_all_conv+(u_all-u_all_conv)*ll/n_ll) dNdXYZ(2:3:24-1)'*(u_all_conv+(u_all-u_all_conv)*ll/n_ll) dNdXYZ(3:3:24)'*(u_all_conv+(u_all-u_all_conv)*ll/n_ll);
                    dNdXYZ(1:3:24-2)'*(v_all_conv+(v_all-v_all_conv)*ll/n_ll) dNdXYZ(2:3:24-1)'*(v_all_conv+(v_all-v_all_conv)*ll/n_ll) dNdXYZ(3:3:24)'*(v_all_conv+(v_all-v_all_conv)*ll/n_ll);
                    dNdXYZ(1:3:24-2)'*(w_all_conv+(w_all-w_all_conv)*ll/n_ll) dNdXYZ(2:3:24-1)'*(w_all_conv+(w_all-w_all_conv)*ll/n_ll) dNdXYZ(3:3:24)'*(w_all_conv+(w_all-w_all_conv)*ll/n_ll)]+eye(3);
                ...
                yieldfunction=sqrt(1.5*(Mdev(1,1)^2+Mdev(2,2)^2+Mdev(3,3)^2+2*Mdev(1,2)^2+2*Mdev(1,3)^2+2*Mdev(2,3)^2))-My0-Hardening_modulus*(epcum+depcum)^Hardening_exponent;
                ...
                if yieldfunction<0
                    ...
                    if epcum==0 && depcum==0 && Hardening_exponent~=1
                        ...
                        yieldfunction=sqrt(1.5*(Mdev(1,1)^2+Mdev(2,2)^2+Mdev(3,3)^2+2*Mdev(1,2)^2+2*Mdev(1,3)^2+2*Mdev(2,3)^2))-My0-Hardening_modulus*(epcum+depcum+depcum_last)^Hardening_exponent;
                    ...
                    iter_RM=0;
                    res_RM_prev=1e100;
                    res_RM=abs(yieldfunction);
                    while res_RM>tol_RM %&& res_RM<res_RM_prev
                        ...
                        dyieldfunctionddepcum_last=-Hardening_modulus*Hardening_exponent*(epcum+depcum+depcum_last)^(Hardening_exponent-1);
...
%         dEdu(3:3:24)=dNdXYZ(1:3:24-2)*P(3,1)+dNdXYZ(2:3:24-1)*P(3,2)+dNdXYZ(3:3:24)*P(3,3);
        ...
        mx=mx+Hardening_modulus/(Hardening_exponent+1)*(epcum+depcum)^(Hardening_exponent+1);
        mx=mx+My0*depcum;
