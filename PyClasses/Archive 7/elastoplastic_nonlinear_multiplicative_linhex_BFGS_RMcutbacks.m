...
for incr=46%1:incr_tot
    ...
    [m_new,f_2,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,u,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv);
    ...
    while (m_old-m_new)>tol*abs(m_old)
        ...
        for i=1:length(ALPHAS)
            ...
            [m_3,f_3,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,ux,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv);
            res(i,:)=[alpha3 m_3 f_3'*h_new norm(f_3)];
        end
        figure(1)
        hold on
        plot(res(:,1),res(:,2),'b')
        ...
        hold on
        plot(res(:,1),res(:,3),'r')
        plot(res(1:length(ALPHAS)-1,1),(res(2:length(ALPHAS),2)-res(1:length(ALPHAS)-1,2))/ALPHAS(2),'g--')
        figure(3)
        hold on
        plot(res(:,1),res(:,4),'k')
        asdf
        ...
        if signal1==0
            ...
            [m_2,f_2,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,ux,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv);
...
function [m,f,FP_temp,DELTA_EPCUM] = m_and_f(f_ex,u,LEN_el,indices_neumann,FE_node_coord,con_mat,Young,nu,My0,Hardening_modulus,Hardening_exponent,FP_conv,EPCUM,tol_RM,u_conv)
...
for ii=1:length(con_mat(:,1))
    ...
    for jj=1:8
        ...
        while cur_time<1
            ...
            F=[dNdXYZ(1:3:24-2)'*(u_all_conv+(u_all-u_all_conv)*cur_time) dNdXYZ(2:3:24-1)'*(u_all_conv+(u_all-u_all_conv)*cur_time) dNdXYZ(3:3:24)'*(u_all_conv+(u_all-u_all_conv)*cur_time);
                dNdXYZ(1:3:24-2)'*(v_all_conv+(v_all-v_all_conv)*cur_time) dNdXYZ(2:3:24-1)'*(v_all_conv+(v_all-v_all_conv)*cur_time) dNdXYZ(3:3:24)'*(v_all_conv+(v_all-v_all_conv)*cur_time);
                dNdXYZ(1:3:24-2)'*(w_all_conv+(w_all-w_all_conv)*cur_time) dNdXYZ(2:3:24-1)'*(w_all_conv+(w_all-w_all_conv)*cur_time) dNdXYZ(3:3:24)'*(w_all_conv+(w_all-w_all_conv)*cur_time)]+eye(3);
...
%         dEdu(3:3:24)=dNdXYZ(1:3:24-2)*P(3,1)+dNdXYZ(2:3:24-1)*P(3,2)+dNdXYZ(3:3:24)*P(3,3);
        ...
        mx=0.5*Ca*(trace(Fe'*Fe)-3-2*log(det(Fe)))+0.5*Cb*log(det(Fe))^2;
